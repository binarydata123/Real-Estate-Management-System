...
export default function Agents() {
  ...
  const [selectedAgent, setSelectedAgent] = useState<AgentFormData | null>(
    null
  );
  ...
  const getAllAgents = useCallback(
    async (page = 1, search = "", status = "", append = false) => {
      try {
        ...
        if (res.success) {
          setAgents((prev) => (append ? [...prev, ...res.data] : res.data));
  ...
  useEffect(() => {
    setAgents([]); // Clear agents when search term changes
    getAllAgents(1, debouncedSearchTerm, debouncedSearchStatus);
  }, [getAllAgents, debouncedSearchTerm, debouncedSearchStatus]);

  const handlePageChange = (page: number) => {
    if (page >= 1 && page <= totalPages && !isFetching) {
      getAllAgents(page, debouncedSearchTerm, debouncedSearchStatus, true);
    }
  };
      ...
      <div className="sm:flex sm:items-center sm:justify-between sm:gap-4">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">
            A list of all the agents in the system including their name,
            members, and status.
          </p>
        </div>
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white dark:bg-gray-900 dark:divide-gray-700">
                          {agents.map((agent) => (
                            <tr key={agent._id}>
                              ...
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                {(agent.customersCount ?? 0) > 0 &&
                                agent.agencyId?._id ? (
                                  <a
                                    className="text-blue-800"
                                    href={`/admin/customers?agencyId=${agent.agencyId._id}`}
                                  >
                              ...
                              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                ...
                                agent.agencyId?._id ? (
                                  <a
                                    className="text-blue-800"
                                    href={`/admin/properties?agencyId=${agent.agencyId._id}`}
                                  >
