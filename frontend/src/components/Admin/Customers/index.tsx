...
const statusStyles: { [key: string]: string } = {
    ...
    interested: 'bg-pink-100 text-pink-800 dark:bg-pink-500/10 dark:text-pink-400',
    negotiating: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-500/10 dark:text-yellow-400',
    converted: 'bg-green-100 text-green-800 dark:bg-green-500/10 dark:text-green-400',
    not_interested: 'bg-red-100 text-red-800 dark:bg-red-500/10 dark:text-red-400',
    follow_up: 'bg-orange-100 text-orange-800 dark:bg-orange-500/10 dark:text-orange-400',
};
            ...
            <div className="sm:flex sm:items-center sm:justify-between sm:gap-4">
                <div className="sm:flex-auto">
                    ...
                    <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">
                        A list of all the customers in the system including their name, members, and status.
                    </p>
                </div>
                                                ...
                                                <tbody className="divide-y divide-gray-200 bg-white dark:bg-gray-900 dark:divide-gray-700">
                                                    {customers.map((customer) => (
                                                        <tr key={customer._id}>
                                                            ...
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                                                {customer.maximumBudget?.toLocaleString("en-US", {
                                                                    style: "currency",
                                                                    currency: "USD",
                                                                    maximumFractionDigits: 0,
                                                                }) || '--'}
                                                            </td>
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">{customer?.agencyId?.name || 'N/A'}</td>
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                                                <span className={classNames(statusStyles[customer.status], 'inline-flex capitalize items-center rounded-full px-2.5 py-0.5 text-xs font-medium')}>
                                                                    {customer.status.replace(/_/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())}
                                                                </span>
                                                            </td>
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
