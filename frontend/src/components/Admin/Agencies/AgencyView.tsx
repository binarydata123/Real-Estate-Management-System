...
export default function AgencyView({ agencyId }: { agencyId: string }) {
  ...
  const SearchInput = ({ value, onChange, ariaLabel }: any) => (
    <input
      type="text"
      placeholder={`Search ${ariaLabel}`}
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className="w-full max-w-lg p-3 rounded-xl border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition-colors dark:bg-gray-700 dark:border-gray-600 dark:text-white"
    />
  );
  ...
  const MeetingCard: React.FC<{ meeting: MeetingFormData }> = ({ meeting }) => {
    const primaryImage = meeting.propertyData?.images?.length
      ? getPropertyImageUrlWithFallback(meeting.propertyData.images.find((img) => img.isPrimary)?.url)
    ...
    const { colorClass, bgColorClass, statusText } = (() => {
      switch (meeting.status) {
        case "completed":
          return { colorClass: "text-green-700", bgColorClass: "bg-green-100", statusText: "Completed" };
        case "cancelled":
          return { colorClass: "text-red-700", bgColorClass: "bg-red-100", statusText: "Cancelled" };
        case "rescheduled":
          return { colorClass: "text-yellow-700", bgColorClass: "bg-yellow-100", statusText: "Rescheduled" };
        case "scheduled":
        default:
          return { colorClass: "text-blue-700", bgColorClass: "bg-blue-100", statusText: "Scheduled" };
      }
    })();
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center space-x-4">
              ...
              <div className="flex-grow min-w-0">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-xs flex items-center mt-0.5">
                  ...
                  <span className="truncate">{meeting.propertyData.location}</span>
                </p>
            ...
            <div className="flex overflow-x-auto no-scrollbar">
              {[
                ...
                <button
                  ...
                  className={`flex-1 min-w-[140px] py-4 px-6 text-center font-medium transition-colors whitespace-nowrap ${activeTab === tab.id
                      ...
                      teamMembers.map((member, index) => (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            {/* Status on Right */}
                            <span
                              className={`px-2 py-1 text-xs rounded-full font-medium ${member.status === "active"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {isLoading && propertiesCurrentPage === 1 ? (
                    ...
                    properties.map((property, index) => {
                      ...
                      const statusColor = property.status === "Available"
                        ? "bg-green-500"
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="h-48 w-full bg-gray-100 relative">
                            ...
                            />
                            {/* Status Badge */}
                            <span className={`absolute top-3 right-3 px-3 py-1 text-xs font-semibold text-white rounded-full shadow-md ${statusColor}`}>
                              {statusText}
                            </span>
                            {/* Category/Type Tag */}
                          ...
                          <div className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3 text-sm text-gray-700 dark:text-gray-300 font-medium">
                              <div className="flex flex-col items-center space-y-1 p-2 bg-blue-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                ...
                                <span className="truncate text-xs">{property.bedrooms} Bed</span>
                              </div>
                              <div className="flex flex-col items-center space-y-1 p-2 bg-blue-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                <BathIcon className="w-4 h-4 text-blue-500 dark:text-indigo-400" />
                                <span className="truncate text-xs">{property.bathrooms} Bath</span>
                              </div>
                              <div className="flex flex-col items-center space-y-1 p-2 bg-blue-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                <RulerIcon className="w-4 h-4 text-blue-500 dark:text-indigo-400" />
                                <span className="truncate text-xs">{property.built_up_area} sq.ft</span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center text-gray-600 dark:text-gray-400 text-sm">
                              ...
                              <span className="truncate">{property.location || "Location not updated"}</span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-y-2 text-xs text-gray-500 dark:text-gray-400 pt-2 border-t border-dashed border-gray-100 dark:border-gray-700">
                              <p className="truncate"><span className="font-semibold text-gray-700 dark:text-gray-200">Facing:</span> {property.facing}</p>
                              <p className="truncate"><span className="font-semibold text-gray-700 dark:text-gray-200">Age:</span> {property.property_age}</p>
                              <p className="truncate"><span className="font-semibold text-gray-700 dark:text-gray-200">Furnishing:</span> {property.furnishing}</p>
                              <p className="truncate"><span className="font-semibold text-gray-700 dark:text-gray-200">RERA:</span> {property.rera_status}</p>
                            </div>

                            {/* Owner Section */}
                            {(property.owner_name || property.owner_contact) && (
                              <div className="pt-3 border-t border-gray-100 dark:border-gray-700 space-y-1">
                                <p className="font-bold text-sm text-gray-800 dark:text-gray-100">Owner Contact</p>
                                {property.owner_name && (
                                  <p className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                                    <User className="w-4 h-4 text-blue-500 mr-2" />
                                    <span className="pl-0 truncate">{property.owner_name}</span>
                                  </p>
                                )}
                                {property.owner_contact && (
                                  <p className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                                    <Phone className="w-4 h-4 text-blue-500 mr-2" />
                                    <span className="pl-0 truncate">{property.owner_contact}</span>
                                  </p>
                                )}
                              </div>
                            )}
                    ...
                    <div className="col-span-full text-center mt-6 p-10 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 mt-2">No properties found matching your criteria.</p>
                    </div>
                  )}
                </div>
            ...
            {activeTab === "meetings" && (
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Client Meetings</h2>
                ...
                <div className="text-sm font-medium text-gray-600 dark:text-gray-400">
                  ...
                  Total Meetings Scheduled: <span className="font-bold text-gray-800 dark:text-white">{meetingsTotalRecords}</span>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {isLoading && meetingsCurrentPage === 1 ? (
                      ...
                      meetings.map((meeting, index) => (
                        <MeetingCard key={meeting._id || index} meeting={meeting} />
                      ))
                    ) : (
                      <div className="col-span-full text-center mt-6 p-10 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
                        <Calendar className="mx-auto w-16 h-16 text-blue-300 dark:text-indigo-400 mb-4" />
                        <p className="text-gray-500 dark:text-gray-400 mt-2">No meetings found.</p>
                      </div>
                    )}
                  </div>
                  <div className="mt-8">
