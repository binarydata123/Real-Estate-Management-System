...
import { XMarkIcon, ShareIcon, UserIcon, UsersIcon } from '@heroicons/react/24/outline';
...
const SharePropertyModal: React.FC<SharePropertyModalProps> = ({ property, onClose }) => {
            ...
            <div className="bg-white rounded-lg md:rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-2 md:p-6">
                    ...
                    <div className="bg-gray-50 rounded-lg md:p-4 md:mb-6 p-2 mb-2">
                        ...
                        <p className="text-lg font-bold text-gray-900 mt-1">{formatPrice(property.price)}</p>
                    </div>
                    ...
                    <form onSubmit={handleShare} className="space-y-2">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button
                                    ...
                                    className={`p-4 border-2 rounded-lg transition-colors ${shareType === 'customer' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                                        }`}
                                >
                                    <UserIcon className="h-8 w-8 mx-auto mb-2 text-gray-600" />
                                    <p className="font-medium text-gray-900">Customer</p>
                                    <p className="text-sm text-gray-600">Share with potential buyer</p>
                                </button>

                                <button
                                    type="button"
                                    onClick={() => setShareType('agent')}
                                    className={`p-4 border-2 rounded-lg transition-colors ${shareType === 'agent' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                                        }`}
                                >
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                {shareType === 'customer' ? 'Customer WhatsApp/Email' : 'Agent Email'}
                            </label>
                            <input
                                type="text"
                                value={recipient}
                                ...
                                placeholder={shareType === 'customer' ? '+91 98765 43210 or email@example.com' : 'agent@example.com'}
                            />
                        </div>
                        ...
                        {shareType === 'agent' && (
                            <div>
                                <label className="flex items-center">
                                    ...
                                    />
                                    <span className="md:ml-2 ml-1 text-sm text-gray-700">Include owner contact details</span>
                                </label>
