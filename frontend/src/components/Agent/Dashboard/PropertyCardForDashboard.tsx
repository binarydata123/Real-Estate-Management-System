...
const PropertyCardForDashboard: React.FC<PropertyCardProps> = ({
    ...
    const primaryImage = property.images?.[0] || 'https://images.pexels.com/photos/106399/pexels-photo-106399.jpeg';
        ...
        <div
            ...
            <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute md:top-3 md:left-3 top-2 left-2">
                    <span className={`inline-flex items-center capitalize px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(property.status)}`}>
                        {property.status}
                    </span>
                </div>
            ...
            <div className="p-1 md:p-4 flex flex-col flex-1">
                <div className="flex-1">
                    <div className="flex justify-between items-start mb-1 md:mb-2">
                        <h3 className="font-semibold text-gray-900 text-base md:text-lg leading-tight pr-2 flex-1 truncate">{property.title}</h3>
                    ...
                    <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs md:text-sm text-gray-600 md:mt-1 md:mt-3">
                        <span className="bg-gray-100 px-2 py-0.5 rounded capitalize">{property.category}</span>
                        {property.size && (
                            <span className="flex items-center">{property.size} {property.size_unit}</span>
                        )}
                        {property.bedrooms && (
                            <span className="flex items-center">{property.bedrooms} BHK</span>
                        )}
                        {property.bathrooms && (
                            <span className="flex items-center">{property.bathrooms} Bath</span>
                        )}
                    </div>
                </div>
