    ...
    const handleInteraction = (
        ...
        const moveHandler = (moveEvent: MouseEvent | TouchEvent) => {
            ...
            if (thumb === 'min') {
                ...
                setValue(minName, newMinVal as UserPreferenceFormData[typeof minName], { shouldDirty: true, shouldValidate: true });
            } else { // thumb === 'max'
                const newMaxVal = Math.max(newValue, minVal + step);
                setValue(maxName, newMaxVal as UserPreferenceFormData[typeof maxName], { shouldDirty: true, shouldValidate: true });
            }
        };
    ...
    return (
        <div className="col-span-2 md:col-span-4">
            ...
            <div className="relative h-10 flex items-center mt-8">
                <div ref={trackRef} className="relative w-full h-1.5 bg-gray-200 rounded-full">
                    <div className="absolute h-full bg-blue-500 rounded-full" style={{ left: `${minPercent}%`, right: `${100 - maxPercent}%` }} />

                    {/* Min Thumb with Indicator */}
                    <div className="absolute top-1/2 -translate-y-1/2" style={{ left: `${minPercent}%`, zIndex: 11 }}>
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2" style={{ left: `${maxPercent}%`, zIndex: 10 }}>
                        <div className="absolute bottom-full mb-2 -translate-x-1/2">
                            <div className="relative bg-gray-800 text-white text-xs font-semibold py-1 px-2 rounded-md whitespace-nowrap">
...
export default function PreferenceForm() {
    ...
    const { register, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm<UserPreferenceFormData>({
    ...
    const filteredFeatures = useMemo(() => {
        if (containsOnlyPlotOrLand) return featuresOptions.filter(opt => opt.value === 'Vaastu Compliant');
        if (watchedCategories.length === 0) return featuresOptions.filter(opt => !opt.categories);
        return featuresOptions.filter(opt => !opt.categories || watchedCategories.some(cat => opt.categories!.includes(cat as (typeof opt.categories extends Array<infer U> ? U : never))));
    }, [watchedCategories, containsOnlyPlotOrLand]);

    const filteredAmenities = useMemo(() => {
        if (containsOnlyPlotOrLand) return [];
        if (watchedCategories.length === 0) return amenitiesOptions.filter(opt => !opt.categories);
        return amenitiesOptions.filter(opt => !opt.categories || watchedCategories.some(cat => opt.categories!.includes(cat as typeof opt.categories extends Array<infer U> ? U : never)));
    }, [watchedCategories, containsOnlyPlotOrLand]);
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2">
                <div>
                    <h1 className="text-3xl font-bold md:mb-2 text-gray-800">Set Your Property Preferences</h1>
