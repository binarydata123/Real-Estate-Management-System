...
export const createProperty = async (req, res) => {
    try {
        ...
        const propertyData = {
            ...
            built_up_area: body.built_up_area ? Number(body.built_up_area) : undefined,
            ...
            images: imageEntries,
            property_code: body.property_code || `PROP-${Date.now()}-${Math.random().toString(36).substring(2, 7).toUpperCase()}`,
        };

        // The 'agency' field is expected to be in the body from the frontend
        if (!propertyData.agencyId) {
            return res.status(400).json({ success: false, message: 'Agency information is missing.' });
        }
