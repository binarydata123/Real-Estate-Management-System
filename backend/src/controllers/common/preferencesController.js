...
export const getPreferenceDetail = async (req, res) => {
    try {
        ...
        const request = await PreferenceRequest.findOne({ sentToUserId: userId, sentByUserId: req.user._id });

        res.status(200).json({ success: true, data: preference, requestSent: request ? true : false });
