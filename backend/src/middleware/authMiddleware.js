...
const protect =
  ...
  async (req, res, next) => {
    ...
    if (
      effectiveRoles.includes("agent") &&
      !effectiveRoles.includes("teamMember")
    ) {
      effectiveRoles.push("teamMember");
    }
    if (
      ...
      try {
        ...
        if (
          Array.isArray(effectiveRoles) &&
          effectiveRoles.length > 0 &&
          !effectiveRoles.includes(req.user.role)
        ) {
          return res
            .status(403)
